...
use Amp\Promise;
use R2WpBaziPlugin\vendor\Psr\Log\LoggerAwareInterface;
use R2WpBaziPlugin\vendor\Psr\Log\LoggerAwareTrait;
...
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Response\AmpResponse;
...
if (!interface_exists(DelegateHttpClient::class)) {
	throw new \LogicException('You cannot use "R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\AmpHttpClient" as the "amphp/http-client" package is not installed. Try running "composer require amphp/http-client:^4.2.1".');
}

if (!interface_exists(Promise::class)) {
	throw new \LogicException('You cannot use "R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\AmpHttpClient" as the installed "amphp/http-client" is not compatible with this version of "symfony/http-client". Try downgrading "amphp/http-client" to "^4.2.1".');
}
...
final class AmpHttpClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface {
	...
	public function request( string $method, string $url, array $options = [] ): ResponseInterface {
		...
		if ($options['http_version']) {
			$request->setProtocolVersions(
				match ( (float) $options['http_version']) {
				1.0 => [ '1.0' ],
				1.1 => [ '1.1', '1.0' ],
				default => [ '2', '1.1', '1.0' ],
				}
