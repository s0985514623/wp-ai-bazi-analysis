...
use Amp\Promise;
use Amp\Success;
use R2WpBaziPlugin\vendor\Psr\Log\LoggerInterface;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Chunk\FirstChunk;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Chunk\InformationalChunk;
...
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\HttpClientTrait;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Internal\AmpBody;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Internal\AmpClientState;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Internal\Canary;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Internal\ClientState;
...
final class AmpResponse implements ResponseInterface, StreamableInterface {
	...
	public function __construct( AmpClientState $multi, Request $request, array $options, ?LoggerInterface $logger ) {
		...
		Loop::defer(
			static function () use ( $request, $multi, $id, &$info, &$headers, $canceller, &$options, $onProgress, &$handle, $logger, &$pause ) {
		...
		$info['pause_handler'] = static function ( float $duration ) use ( &$throttleWatcher, &$pauseDeferred, &$pause ) {
			...
			if ($duration <= 0) {
				...
				$throttleWatcher = Loop::delay(
					ceil(1000 * $duration),
					static function () use ( &$pauseDeferred ) {
						$deferred      = $pauseDeferred;
						$pauseDeferred = new Deferred();
						$deferred->resolve();
					}
		...
		$this->canary = new Canary(
			static function () use ( $canceller, $multi, $id ) {
				$canceller->cancel();
	...
	private static function generateResponse( Request $request, AmpClientState $multi, string $id, array &$info, array &$headers, CancellationTokenSource $canceller, array &$options, \Closure $onProgress, &$handle, ?LoggerInterface $logger, Promise &$pause ): \Generator {
		$request->setInformationalResponseHandler(
			static function ( Response $response ) use ( $multi, $id, &$info, &$headers ) {
