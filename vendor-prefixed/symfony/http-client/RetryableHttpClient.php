...
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Retry\GenericRetryStrategy;
use R2WpBaziPlugin\vendor\Symfony\Component\HttpClient\Retry\RetryStrategyInterface;
...
class RetryableHttpClient implements HttpClientInterface, ResetInterface {
	...
	public function request( string $method, string $url, array $options = [] ): ResponseInterface {
		...
		return new AsyncResponse(
			...
			$url,
			$options,
			function ( ChunkInterface $chunk, AsyncContext $context ) use ( $method, $url, $options, $maxRetries, &$baseUris ) {
				static $retryCount = 0;
				static $content    = '';
				static $firstChunk;
				...
				$this->logger?->info(
				'Try #{count} after {delay}ms' . ( $exception ? ': ' . $exception->getMessage() : ', status code: ' . $context->getStatusCode() ),
				[
					'count' => $retryCount,
					'delay' => $delay,
				]
